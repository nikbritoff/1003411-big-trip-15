import { OPTION_TITLES } from '../mock/event.js';

const setOptions = (options, type) => {
  let avialableOptins = '';
  if (OPTION_TITLES[type] !== undefined) {
    top: for (let i = 0; i < OPTION_TITLES[type].options.length; i++) {
      const option = OPTION_TITLES[type].options[i];
      for (let j = 0; j < options.length; j++) {
        const selectedOption = options[j];
        if (option.title === selectedOption.title) {
          avialableOptins = `${avialableOptins}
          <div class="event__offer-selector">
            <input class="event__offer-checkbox  visually-hidden" id="event-offer-comfort-1" type="checkbox" name="event-offer-comfort" checked>
            <label class="event__offer-label" for="event-offer-comfort-1">
              <span class="event__offer-title">${selectedOption.title}</span>
              &plus;&euro;&nbsp;
              <span class="event__offer-price">${selectedOption.price}</span>
            </label>
          </div>`;
          continue top;
        }
...
const createAddTripEventTemplate = (data) => {
  const {type, destination, description, options, basePrice, dateFrom, dateTo, pictures} = data;
...
const createEditTripEventTemplate = (data) => {
  const {type, destination, description, options, basePrice, dateFrom, dateTo} = data;
...
export {createAddTripEventTemplate, createEditTripEventTemplate};
